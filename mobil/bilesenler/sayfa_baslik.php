...
if (isset($kullanici_kim['id']))
{
	...
	while ($bildirim = $vt->fetch_assoc($bilsonuc))
	{
		...
		elseif ($bildirim['tip'] == '4')
		{
			...
			$okunmamis_oi3 .= '<li><a href="'.$dizin_bilgi.'konu.php?'.$tsk_konu[1].'" onclick="BKapat(1)" title="Teşekkür" id="bilyazi">
			<h6><u>Teşekkür edildi</u></h6>
			<p>Eden: '.$tsk_konu[0].'</p>
			<p>'.zonedate($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $bildirim['tarih']).
			'</p></a></li>
			...
			// --></script>
			<script type="text/javascript" src="'.$dizin_bilgi.'dosyalar/betik_bildirim.js"></script>';
		}
	}
	...
	if (($ayarlar['o_ileti'] == 1) AND ($kullanici_kim['okunmamis_oi']))
	{
		...
		$vtsorgu = "SELECT id,ozel_baslik,kimden,gonderme_tarihi,okunma_tarihi FROM $tablo_ozel_ileti WHERE
		kime='$kullanici_kim[kullanici_adi]' AND alan_kutu='1' AND okunma_tarihi is null
