...
$dosya_adi = 'forum_ip_yonetimi.php';
...
FROM $tablo_pyorumlar WHERE yazan_ip='$_GET[ip]'
...
$sayfa_aciklama3 = '<br><div align="center" style="text-align: left; width: 100%; height: 18px;"><a target="_blank" href="http://www.whois.sc/'.$_GET['ip'].'"><b>'.$_GET['ip'];
if ($satir_sayi>100){
...
$sayfa_aciklama = '<br><div align="center" style="text-align: left; width: 699px; height: 18px;">
<a href="'.$dosya_adi.'" style="text-decoration: none;"><b>&laquo; &nbsp;ilk sayfasına geri dön</b></a></div>';
...
if ($vt->num_rows($vtsonuc3))
{
	...
	while ($uye = $vt->fetch_assoc($vtsonuc3))
	{
		...
		$kayit = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $uye['katilim_tarihi'], $ayarlar['tarih'], false);
		$son_giris = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $uye['son_hareket'], $ayarlar['tarih'], false);
...
if ($vt->num_rows($vtsonuc4))
{
	...
	while ($uye = $vt->fetch_assoc($vtsonuc4))
	{
		...
		$kayit = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $uye['giris'], $ayarlar['tarih'], false);
		$son_giris = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $uye['son_hareket'], $ayarlar['tarih'], false);
		$hsayfa = HangiSayfada($uye['sayfano'], $uye['hangi_sayfada']);
		$hsayfa = str_replace('<a href="', '<a href="../', $hsayfa);
...
if ($vt->num_rows($vtsonuc1))
{
	...
	while ($konular = $vt->fetch_assoc($vtsonuc1))
	{
		...
		if ($konular['rakam'] == '9')
		{
			...
			$cevap_tarihi = '<center>'.zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $konular['tarih'], $ayarlar['tarih'], false).'</center>';


			if ($konular['rakam2'] == '9')
			{
				$islem = 'Cevap Yazma';
				$tarih = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $konular['tarih'], $ayarlar['tarih'], false);
			...
			else
			{
				...
				$tarih = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $konular['degistirme_tarihi'], $ayarlar['tarih'], false);
		...
		elseif ($konular['rakam'] == '6')
		{
			...
			$islem = 'Yorum Yazma';
			$tarih =  zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $konular['tarih'], $ayarlar['tarih'], false);
			$ip_adresi = '<a href="'.$dosya_adi.'?kip=1&amp;ip='.$konular['yazan_ip'].'">'.$konular['yazan_ip'].'</a>';
		}
...
$kayit_ip = 'Kayıt Tarihi';
$yazan_ip = 'Yazan';
$ip_degistiren = 'İşlem Yapan';
$uye_misafir = 'Üyeler ve Misafirler';
$konular_cevaplar = 'Bu ip Adresiyle Yapılan Son işlemler';
...
UNION ALL SELECT id,yazan as yazan,yazan_id,tarih,silinmis,uye_adi as mesaj_baslik,uye_id as hangi,yazan_ip,onay,tarih as tarih2,6 AS rakam,6 AS rakam2
FROM $tablo_pyorumlar WHERE yazan_id='$kullanici[id]'
...
$sayfa_aciklama3 = '<br><div align="center" style="text-align: left; width: 100%; height: 18px;"><b>'.$kullanici['kullanici_adi'];
if ($satir_sayi>100){
...
if ($vt->num_rows($vtsonuc3))
{
	...
	$kayit = '<a href="'.$dosya_adi.'?kip=1&amp;ip='.$uye['kul_ip'].'">'.$uye['kul_ip'].'</a>';
	$son_giris = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $uye['son_hareket'], $ayarlar['tarih'], false);
	$hsayfa = HangiSayfada($uye['sayfano'], $uye['hangi_sayfada']);
	$hsayfa = str_replace('<a href="', '<a href="../', $hsayfa);
	...
	while ($konular = $vt->fetch_assoc($vtsonuc1))
	{
		...
		if ($konular['rakam'] == '9')
		{
			...
			else
			{
				$islem = 'Cevap Değiştirme';
				$tarih = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $konular['degistirme_tarihi'], $ayarlar['tarih'], false);
				$ip_adresi = '<a href="'.$dosya_adi.'?kip=1&amp;ip='.$konular['degistiren_ip'].'">'.$konular['degistiren_ip'].'</a>';
			}
		}
...
$kayit_ip = 'IP Adresi';
$ip_degistiren = 'IP Adresi';
$uye_misafir = 'Üyenin Bilgileri';
$konular_cevaplar = 'Üyenin Yaptığı Son işlemler';
...
if (isset($_SERVER["HTTP_CF_CONNECTING_IP"])) $ip_adresi = $_SERVER["HTTP_CF_CONNECTING_IP"];
else $ip_adresi = $_SERVER['REMOTE_ADDR'];
$ip_adresi = zkTemizle4($ip_adresi);
$ip_adresi = zkTemizle($ip_adresi);
...
if (isset($safya_kip))
{
	...
	$sayfalama .= '
	<table cellspacing="1" cellpadding="8" border="0" align="right" class="tablo-ana" style="margin:20px 0">
		<tbody>
		<tr class="tablo-baslik">
		<td>Toplam '.$toplam_sayfa.' Sayfa:&nbsp;</td>
	';

	if ($sayfa != 0)
	{
		$sayfalama .= '<td bgcolor="#ffffff" title="ilk sayfaya git">';
		$sayfalama .= '&nbsp;<a href="'.$dosya_adi.'?'.$safya_kip.'">&laquo;ilk</a>&nbsp;</td>';

		$sayfalama .= '<td bgcolor="#ffffff" title="önceki sayfaya git">';
		$sayfalama .= '&nbsp;<a href="'.$dosya_adi.'?'.$safya_kip.'&amp;sayfa='.($sayfa - $adim).'">&lt;</a>&nbsp;</td>';
	}
	...
	for ($sayi=0,$sayfa_sinir=$sayfa; $sayi < $toplam_sayfa; $sayi++)
	{
		...
		else
		{
			...
			else
			{
				...
				$sayfalama .= '&nbsp;<a href="'.$dosya_adi.'?'.$safya_kip.'&amp;sayfa='.($sayi * $adim).'">'.($sayi + 1).'</a>&nbsp;</td>';
			}
		}
	}
	...
	if ($sayfa < ($satir_sayi - $adim))
	{
		...
		$sayfalama .= '&nbsp;<a href="'.$dosya_adi.'?'.$safya_kip.'&amp;sayfa='.(($toplam_sayfa - 1) * $adim).'">son&raquo;</a>&nbsp;</td>';
	}
...
$sayfa_adi = 'IP Yönetimi'; //IP adresine veya üye adına göre arama
$tema_sayfa_baslik = 'IP Yönetimi';
eval(phpkf_tema_yukle('phpkf-bilesenler/temalar/'.$temadizini_cms.'/forum_ip_yonetimi.php'));
