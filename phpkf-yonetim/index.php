...
if ( ((isset($_GET['duyuru'])) AND ($_GET['duyuru'] == '1' )) OR ((isset($_GET['surum'])) AND ($_GET['surum'] == '1' )) )
{
	$ek = 'l='.$site_dili;

	if ($cms_kullan == 0)
	{
		if (isset($_GET['duyuru'])) $ek2 = '&duyuru=forum';
		elseif (isset($_GET['surum'])) $ek2 = '&fsurum='.$surum;
	}
	else
	{
		if (isset($_GET['duyuru'])) $ek2 = '&duyuru=cms';
		elseif (isset($_GET['surum'])) $ek2 = '&csurum='.$surum;
	}

	if (!isset($_SERVER['HTTP_REFERER'])) $_SERVER['HTTP_REFERER'] = $protocol.'://'.$ayarlar['alanadi'].$ayarlar['dizin'].'/phpkf-yonetim/index.php';
...
$out = "GET /resmi_duyuru.php?$ek".$ek2." HTTP/1.1
Accept: text/html
...
";
	...
	{
		if ( (isset($_GET['duyuru'])) AND ($_GET['duyuru'] != '' ) )
		echo mb_convert_encoding('<iframe src="//www.phpkf.com/resmi_duyuru.php?'.$ek.$ek2.'" name="duyuru" style="border:2px solid #ddd; width:800px; height:500px">'.$ly['noiframe'].'</iframe>',"ISO-8859-9","UTF-8");
		exit();
	}
...
adres = \'index.php?duyuru=1\';
var katman1 = document.getElementById(katman);
katman1.innerHTML = \'<img src="../phpkf-dosyalar/yukleniyor2.gif" width="220" height="19" alt="." title="\'+jsl["yukleniyor"]+\'">\';
setTimeout("GonderAl(\'"+adres+"\',\'"+katman+"\')",1000);}';
...
{
	$register_globals = '<font color="#ff0000">'.$ly['register_globals_hata'][0].'<br><a href="http://www.phpkf.com/k820-registerglobals-evrensel-kayit-ozelligini-kapatma.html" target="_blank">'.$ly['register_globals_hata'][1].'</a></font>';
}
...
$vtsorgu = "SHOW TABLE STATUS LIKE '$tablo_oneki%'";
$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());

$toplam_boyut = 0;
while ($tablo_bilgileri = $vt->fetch_assoc($vtsonuc))
...
$acilis_tarihi = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $ayarlar['site_acilis'], $ayarlar['tarih'], false);
$surum_denetle = '<a href="javascript:void(0);" onclick="yenile(\'katman_surum2\', \'1\')">'.$ly['surumu_denetle'].'</a>';
$phpkf_surum = $surum.' &nbsp; ';
...
$sayfa_adi = $ly['anasayfa'];
$tema_sayfa_baslik = $ly['yonetim_anasayfa'];
