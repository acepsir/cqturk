...
$vtsorgu = "SELECT id FROM $tablo_kategoriler ORDER BY id";
$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());

while($kategori = $vt->fetch_assoc($vtsonuc))
{
	// kategorideki yazıların toplamı alınıyor
	$vtsorgu2 = "SELECT id FROM $tablo_yazilar WHERE kategori like '%,$kategori[id],%'";
	$vtsonuc2 = $vt->query($vtsorgu2) or die ($vt->hata_ver());
	$toplam_yazi = $vt->num_rows($vtsonuc2);

	$vtsorgu3 = "UPDATE $tablo_kategoriler SET toplam='$toplam_yazi' WHERE id='$kategori[id]' LIMIT 1";
	$vtsonuc3 = $vt->query($vtsorgu3) or die($vt->hata_ver());
}


header('Location: cms_vt_guncelle.php?kip=guncellendi');
exit();
...
{
	$tema_sayfa_icerik = '<br><center><font color="green"><b>'.$ly['yazilar_guncellendi'].'</b></font></center><br><br>';
}
else
{
	$tema_sayfa_icerik = '<center><br>'.$ly['yazi_sayi_guncelle_bilgi'].'<br><br><br>
	<form name="ayarlar" action="cms_vt_guncelle.php" method="get">
