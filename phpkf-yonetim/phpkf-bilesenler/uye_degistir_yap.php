...
$vtsorgu = "SELECT id,kullanici_adi,sifre,posta,resim FROM $tablo_kullanicilar WHERE id='$uye_id' LIMIT 1";
...
{
	header('Location: ../hatalar.php?hata=31');
	exit();
}

if ( strlen($_POST['gercek_ad']) > 30)
{
	header('Location: ../hatalar.php?hata=32');
	exit();
}
...
if ($_POST['sehir'] != '')
{
	...
	{
		header('Location: ../hatalar.php?hata=36');
		exit();
	}
}
...
{
	header('Location: ../hatalar.php?hata=74');
	exit();
}

if (!preg_match('/^[012]+$/', $_POST['cinsiyet']))
{
	header('Location: ../hatalar.php?hata=92');
	exit();
}
...
{
	header('Location: ../hatalar.php?hata=204');
	exit();
}
...
if ( strlen($_POST['web']) > 100)
{
	header('Location: ../hatalar.php?hata=75');
	exit();
}
...
{
	header('Location: ../hatalar.php?hata=78');
	exit();
}

if ( strlen($_POST['hakkinda']) > 1000 )
{
	header('Location: ../hatalar.php?hata=224');
	exit();
}
...
{
	header('Location: ../hatalar.php?hata=79');
	exit();
}

if ( strlen($_POST['aim']) > 99)
{
	header('Location: ../hatalar.php?hata=80');
	exit();
}

if ( strlen($_POST['msn']) > 99)
{
	header('Location: ../hatalar.php?hata=81');
	exit();
}

if ( strlen($_POST['yahoo']) > 99)
{
	header('Location: ../hatalar.php?hata=82');
	exit();
}

if ( strlen($_POST['skype']) > 99)
{
	header('Location: ../hatalar.php?hata=83');
	exit();
}
	...
	AND ($_FILES['resim_yukle']['tmp_name'] != '') )
{
	...
	if ( (isset($tip)) AND ($tip == 2) )
	{
		...
		{
			header('Location: ../hatalar.php?hata=84');
			exit();
		}
	}
	...
	{
		header('Location: ../hatalar.php?hata=85');
		exit();
	}
	...
	{
		header('Location: ../hatalar.php?hata=88');
		exit();
	}
...
elseif ( ($ayarlar['uye_resim_galerisi'] == 1) AND (isset($_POST['uzak_resim2'])) AND ($_POST['uzak_resim2'] != '') )
{
	...
	{
		header('Location: ../hatalar.php?hata=89');
		exit();
	}
...
if ($satir['posta'] != $_POST['posta'])
{
	...
	{
		header('Location: ../hatalar.php?hata=93');
		exit();
	}
}
...
header('Location: ../hatalar.php?bilgi=32&no='.$satir['id']);
exit();
