...
$genel_cikti = "\n";
$genel_cikti .= '--		'.$ayarlar['site_adi'].' VERiTABANI YEDEGi';
$genel_cikti .= "\n";
$genel_cikti .= '--		TARiH: '.zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, time());
$genel_cikti .= "\n";
$genel_cikti .= '--		SUNUCU ADRESi: '.$TEMA_SITE_ANADIZIN;
$genel_cikti .= "\n\n";
...
if ( (isset($_POST['gzip'])) AND ($_POST['gzip'] == 1) )
{
	...
	header('Content-Type: application/x-gzip; name="phpkf_vt_yedek_'.zaman('d-m-Y', $ayarlar['saat_dilimi'], false, time()).'.sql.gz"');
	header('Content-disposition: attachment; filename=phpkf_vt_yedek_'.zaman('d-m-Y', $ayarlar['saat_dilimi'], false, time()).'.sql.gz');
...
else
{
	...
	header('Content-Type: text/x-delimtext; name="phpkf_vt_yedek_'.zaman('d-m-Y', $ayarlar['saat_dilimi'], false, time()).'.sql"');
	header('Content-disposition: attachment; filename=phpkf_vt_yedek_'.zaman('d-m-Y', $ayarlar['saat_dilimi'], false, time()).'.sql');
