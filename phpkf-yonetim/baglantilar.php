...
if ( (isset($_POST['kip'])) AND ($_POST['kip'] == 'uygula') )
{
	...
	header('Location: hatalar.php?bilgi=18&git='.$git);
	exit();
}
...
elseif ( (isset($_POST['kip'])) AND ($_POST['kip'] == 'yeni') )
{
	...
	{
		header('Location: hatalar.php?hata=2');
		exit();
	}
...
if ( (isset($_GET['kip'])) AND ($_GET['kip'] != '') )
{
	...
	{
		$yer = 2;
		$tema_sayfa_baslik = $ly['blok_baglantilari'];
	...
	{
		$yer = 3;
		$tema_sayfa_baslik = $ly['taban_baglantilari'];
	...
	{
		$yer = 1;
		$tema_sayfa_baslik = $ly['menu_baglantilari'];
...
{
	global $ayarlar, $vt, $site_dili, $tablo_baglantilar, $yo, $lmenu, $cms_kullan, $forum_kullan, $portal_kullan, $_GET;

	$duzenle = '<a href="baglantilar.php?kip='.$_GET['kip'].'&amp;duzenle=1&amp;id='.$baglanti['id'].'&amp;yo='.$yo.'#duzenle" style="text-align:right; float:right;"><img src="phpkf-bilesenler/temalar/varsayilan/resimler/duzenle.png" width="14" height="14" class="duzeltLink" /></a>';
	...
	elseif ($baglanti['ad'] == 'cms') $ad = $lmenu['cms'];
	...
	// CMS, Forum, Portal bağlantıları için ek sorgu
	$drm_syf_ek = '';
...
// cms, forum ve portal için ek sorgu
$ek_sorgu = '';
if ($cms_kullan != 1) $ek_sorgu .= " AND ad!='cms' AND ad!='kategoriler' AND ad!='sayfalar' AND ad!='galeriler' AND ad!='videolar' AND ad!='etiket' AND ad!='iletisim'";
if ($forum_kullan != 1) $ek_sorgu .= " AND ad!='forum' AND ad!='ozel' AND ad!='yardim' AND ad!='mobil' AND ad!='rss'";
...
<input class="input-text" type="text" name="ad_'.$dil.'" value="'.$a.'" style="width:90%" /></div>';
	}
}
...
<legend>'.$tema_sayfa_baslik.'</legend>
...
<input class="input-text" type="text" name="ad" value="'.$bagad.'" onkeyup="SefYapKat(this.value)" style="width:90%" />
</div>
'.$diger_diller.'

<div class="phpkf-form-label">
<label class="label">'.$ly['bilgi'].'<br /></label>
<input class="input-text" type="text" name="bilgi" value="'.$bagbilgi.'" style="width:90%" />
</div>

<div class="phpkf-form-label">
<label class="label">'.$ly['sef_adres'].'<br /></label>
<input class="input-text" type="text" name="adres" value="'.$bagadres.'" style="width:90%" />
</div>
