...
if ($_GET['kullanici'] == 'etkisiz')
{
	if ($_GET['kul_id'] == 1)
	{
		header('Location: hatalar.php?hata=61');
		exit();
	}
	...
	header('Location: hatalar.php?bilgi=33');
	exit();
}
...
elseif ($_GET['kullanici'] == 'etkin')
{
	...
	header('Location: hatalar.php?bilgi=25');
	exit();
}
...
elseif ($_GET['kullanici'] == 'sil')
{
	if ($_GET['kul_id'] == 1)
	{
		header('Location: hatalar.php?hata=137');
		exit();
	}
	...
	if ((isset($_GET['kip'])) AND ($_GET['kip'] == 'engelli'))
		header('Location: hatalar.php?bilgi=23');

	elseif ((isset($_GET['kip'])) AND ($_GET['kip'] == 'etkisiz'))
		header('Location: hatalar.php?bilgi=26');

	else header('Location: hatalar.php?bilgi=34');
	exit();
}
...
elseif ($_GET['kullanici'] == 'engelle')
{
	if ($_GET['kul_id'] == 1)
	{
		header('Location: hatalar.php?hata=149');
		exit();
	}
	...
	header('Location: hatalar.php?bilgi=35');
	exit();
}
...
elseif ($_GET['kullanici'] == 'engel_kaldir')
{
	...
	header('Location: hatalar.php?bilgi=24');
	exit();
}
...
if ((isset($_GET['kip'])) AND ($_GET['kip'] == 'engelli'))
{
	...
	$tema_sayfa_baslik = $ly['engellenmis_uyeler'];
...
elseif ((isset($_GET['kip'])) AND ($_GET['kip'] == 'etkisiz'))
{
	...
	$tema_sayfa_baslik = $ly['etkin_olmayan_uyeler'];
...
else
{
	...
	$tema_sayfa_baslik = $ly['etkin_uyeler'];
...
// Simgeler
$duzenle_simge = '<img src="phpkf-bilesenler/temalar/varsayilan/resimler/duzenle.png" width="16" height="16" alt="d" style="margin-left:6px; margin-right:12px" title="'.$ly['yetki_profil_degistir'].'" />';

$sil_simge = '<img src="phpkf-bilesenler/temalar/varsayilan/resimler/sil.png" width="15" height="15" alt="s" style="margin-left:12px; margin-right:6px" title="'.$l['sil'].'" />';


$etkin_simge = '<img src="phpkf-bilesenler/temalar/varsayilan/resimler/bos2.png" width="15" height="15" alt="a" style="margin-left:6px; margin-right:12px" title="'.$ly['etkisiz_yap'].'" />';

$etkisiz_simge = '<img src="phpkf-bilesenler/temalar/varsayilan/resimler/dogru.png" width="15" height="15" alt="a" style="margin-left:6px; margin-right:12px" title="'.$ly['etkin_yap'].'" />';


$engelli_simge = '<img src="phpkf-bilesenler/temalar/varsayilan/resimler/bos.png" width="15" height="15" alt="b" style="margin-left:12px; margin-right:6px" title="'.$ly['engel_kaldir'].'" />';

$engelsiz_simge = '<img src="phpkf-bilesenler/temalar/varsayilan/resimler/yanlis.png" width="15" height="15" alt="b" style="margin-left:12px; margin-right:6px" title="'.$ly['engelle'].'" />';
...
$uye_adi = '&nbsp;<a href="../'.$phpkf_dosyalar['profil'].'?u='.$uyeler_satir['id'].'">'.$uyeler_satir['kullanici_adi'].'</a>';

$uye_katilim = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $uyeler_satir['katilim_tarihi'], $ayarlar['tarih'], true);

$uye_son_hareket = zaman($ayarlar['tarih_bicimi'], $ayarlar['saat_dilimi'], false, $uyeler_satir['son_hareket'], $ayarlar['tarih'], true);

$uye_ip = $uyeler_satir['kul_ip'];

$uye_mesaj = NumaraBicim($uyeler_satir['mesaj_sayisi']);

$yetki = phpkf_tema_yetkiler($uyeler_satir['id'], $uyeler_satir['yetki'], 1);
$uye_yetki = '<span class="'.$yetki['renk'].'">'.$yetki['isim'].'</span>';


$uye_duzenle = '<a href="uye_degistir.php?u='.$uyeler_satir['id'].'">'.$duzenle_simge.'</a>';
...
$uyeler_dongu .= '<tr class="tablo-satir">
...
$sayfalama .= '<p>
<table cellspacing="1" cellpadding="5" border="0" align="right" bgcolor="#cccccc">
	<tr>
	<td bgcolor="#ffffff">
