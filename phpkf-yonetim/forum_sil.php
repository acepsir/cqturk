...
elseif ( (isset($_POST['fno'])) AND ($_POST['fno'] != '') )
{
	...
	elseif ( (!empty($_POST['tasi'])) AND (!empty($_POST['forumlar'])) )
	{
		...
		$vtsorgu = "UPDATE $tablo_forumlar SET konu_sayisi='$konu_sayi',cevap_sayisi='$cevap_sayi' WHERE id='$_POST[forumlar]' LIMIT 1";
		$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());


		// mesajları silinen forumun konu ve cevap sayısı sıfırlanıyor
		$vtsorgu = "UPDATE $tablo_forumlar SET konu_sayisi='0',cevap_sayisi='0' WHERE id='$_POST[fno]' LIMIT 1";
		$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());
	...
	elseif ( (!empty($_POST['dalatasi'])) AND (!empty($_POST['dalatasi_no'])) )
	{
		...
		if ($tasinan_forum['alt_forum'] == '0')
		{
			...
			$vtsorgu = "SELECT id FROM $tablo_forumlar WHERE dal_no='$tasinan_forum[dal_no]' AND alt_forum='0' AND sira > '$tasinan_forum[sira]'";
			$vtsonuc_sira = $vt->query($vtsorgu) or die ($vt->hata_ver());
		...
		else
		{
			...
			$vtsorgu = "SELECT id FROM $tablo_forumlar WHERE alt_forum='$tasinan_forum[alt_forum]' AND sira > '$tasinan_forum[sira]'";
			$vtsonuc_sira = $vt->query($vtsorgu) or die ($vt->hata_ver());
...
$tema_sayfa_icerik = '<table cellspacing="1" width="580" cellpadding="4" border="0" align="center">
	<tr class="tablo_ici">
...
{
	$tema_sayfa_icerik .= '<input name="dalno" type="hidden" value="'.$_GET['dalno'].'" />
...
<select name="dallar" class="input-select">
<option value="" selected="selected"> &nbsp; - Taşıyacağınız forum dalını seçiniz - &nbsp; ';
	...
	while ($dallar_satir = $vt->fetch_array($dallar_sonuc))
		$tema_sayfa_icerik .= '<option value="'.$dallar_satir['id'].'">'.$dallar_satir['ana_forum_baslik'];
}


elseif ( (isset($_GET['kip'])) AND ($_GET['kip'] == 'forum_sil') )
{
	$tema_sayfa_icerik .= '<input name="fno" type="hidden" value="'.$_GET['fno'].'" />
...
<select name="dalatasi_no" class="input-select">
<option value="" selected="selected"> &nbsp; - Taşıyacağınız forum dalını seçiniz - &nbsp; ';
	...
	while ($dallar_satir2 = $vt->fetch_array($dallar_sonuc))
		$tema_sayfa_icerik .= '<option value="'.$dallar_satir2['id'].'">'.$dallar_satir2['ana_forum_baslik'];


	$tema_sayfa_icerik .= '</select>
<br><br>
<input class="dugme dugme-mavi" name="dalatasi" type="submit" value="Taşı" />
<br><br><br>
...
<br><br>
<select name="forumlar" class="input-select" size="15" style="height:unset">
...
<br><br>
<input class="dugme dugme-mavi" name="tasi" type="submit" value="Buraya Taşı" />
&nbsp; &nbsp;
<input class="dugme dugme-mavi" name="sil" type="submit" value="Taşımadan Sil" />
</center>
</form>
<br><br>
	</td>
...
$sayfa_adi = 'Forum Silme / Taşıma';
$tema_sayfa_baslik = 'Forum Silme / Taşıma';
