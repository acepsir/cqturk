...
if ( (isset($_GET['temadizini'])) AND ($_GET['temadizini'] != '') )
{
	...
	if ($kip == 'forum')
	{
		...
		if (!preg_match('/'.$ayarlar['surum'].'/', $_GET['surum']))
		{
	...
	else
	{
		...
		if (!preg_match('/'.$portal_ayarlar['sayi'].'/', $_GET['surum']))
		{
			header('Location: hata.php?hata=196');
			exit();
		}
...
while ( @gettype($bilgi = @readdir($dizin)) != 'boolean' )
{
	if ( (@is_dir($dizin_adi.$bilgi)) AND ($bilgi != '.') AND ($bilgi != '..') )
	{
		...
		if ($kip == 'forum')
		{
			...
			$tema_uygulama = '<a href="tema_secim.php?kip=forum&amp;temadizini='.$bilgi.'&amp;surum='.$tema_surum[1][0].'&amp;yo='.$yo.'" onclick="return window.confirm(\'Forumun varsayılan temasını değiştirmek istediğinize eminmisiniz ?\')">- varsayılan tema yap -</a>';
			$tema_kullanici = '<a href="tema_secim.php?kip=forum&amp;kullanici='.$bilgi.'&amp;surum='.$tema_surum[1][0].'&amp;yo='.$yo.'" onclick="return window.confirm(\'Tüm üye seçimlerini bu tema ile değiştirmek istediğinize eminmisiniz ?\')">- üye seçimlerini değiştir -</a>';


			if (preg_match("/$bilgi,/", $ayarlar['tema_secenek']))
				$ekle_kaldir = '<a href="tema_secim.php?kip=forum&amp;kaldir='.$bilgi.'&amp;yo='.$yo.'">-KALDIR-</a>';
			else $ekle_kaldir = '<a href="tema_secim.php?kip=forum&amp;ekle='.$bilgi.'&amp;surum='.$tema_surum[1][0].'&amp;yo='.$yo.'">- EKLE -</a>';
			if ($suanki_tema['deger'] == $bilgi) $ekle_kaldir = '<font color="green">Kullanılan</font><br><br>'.$ekle_kaldir;


			if (!preg_match('/'.$ayarlar['surum'].'/', $tema_surum[1][0]))
		...
		elseif ($kip == 'portal')
		{
			...
			$tema_uygulama = '<a href="tema_secim.php?kip=portal&amp;temadizini='.$bilgi.'&amp;surum='.$ptemas.'&amp;yo='.$yo.'" onclick="return window.confirm(\'Portalın varsayılan temasını değiştirmek istediğinize eminmisiniz ?\')">- varsayılan tema yap -</a>';
			$tema_kullanici = '<a href="tema_secim.php?kip=portal&amp;kullanici='.$bilgi.'&amp;surum='.$ptemas.'&amp;yo='.$yo.'" onclick="return window.confirm(\'Tüm üye seçimlerini bu tema ile değiştirmek istediğinize eminmisiniz ?\')">- üye seçimlerini değiştir -</a>';


			if (preg_match("/$bilgi,/", $portal_temalari['sayi']))
				$ekle_kaldir = '<a href="tema_secim.php?kip=portal&amp;kaldir='.$bilgi.'&amp;yo='.$yo.'">-KALDIR-</a>';
			else $ekle_kaldir = '<a href="tema_secim.php?kip=portal&amp;ekle='.$bilgi.'&amp;surum='.$ptemas.'&amp;yo='.$yo.'">- EKLE -</a>';
			if ($suanki_tema['sayi'] == $bilgi) $ekle_kaldir = '<font color="green">Kullanılan</font><br><br>'.$ekle_kaldir;


			if (!preg_match('/'.$portal_ayarlar['sayi'].'/', $ptemas))
...
if ($kip == 'forum')
{
	...
	$sayfa_aciklama = '
<b>&nbsp; &nbsp; /phpkf/temalar/</b> dizininde yüklü olan temalar aşağıda sıralanmaktadır. Yeni tema yüklemek için indirdiğiniz temayı klasörüyle beraber /phpkf/temalar/ dizine kopyalayın ve aşağıdan <b>- varsayılan tema yap -</b> bağlantısını tıklayın. 
<br><br>
...
<br>&nbsp; &nbsp; İstediğiniz temanın yanındaki <b>- üye seçimlerini değiştir -</b> bağlantısını tıklayarak, tüm üyelerin seçimlerini bu tema ile değiştirebilirsiniz.';
	...
	$kip_portal = '<a href="tema_secim.php?kip=portal" style="text-decoration: none"><b>Portal Temaları&nbsp; &raquo;</b></a>';
...
elseif ($kip == 'portal')
{
	...
	$sayfa_aciklama = '
&nbsp; &nbsp; Portal için; &nbsp; <b>/phpkf/portal/temalar/</b> dizininde yüklü olan temalar aşağıda sıralanmaktadır. Yeni tema yüklemek için tek yapmanız gereken temayı klasörüyle beraber bu dizine kopyalamak ve aşağıdan <b>- varsayılan tema yap -</b> bağlantısını tıklamak.
<br><br>
&nbsp; &nbsp; Kullanıcıların yüklediğiniz temalar arasından seçim yapabilmesi için, temanın sol tarafındaki <b>- EKLE -</b> bağlantısı tıklayın. Seçenekler arasından çıkartmak içinse yine aynı yerdeki <b>-KALDIR-</b> bağlantısını tıklayın.
...
<br><br>&nbsp; &nbsp; İstediğiniz temanın yanındaki <b>- üye seçimlerini değiştir -</b> bağlantısını tıklayarak, tüm üyelerin seçimlerini bu tema ile değiştirebilirsiniz.';
	...
	$kip_forum = '<a href="tema_secim.php?kip=forum" style="text-decoration: none"><b>&laquo; &nbsp;Forum Temaları</b></a>';
...
$tema_dosyasi = 'temalar/'.$temadizini.'/tema_secim.php';
