...
elseif ($_POST['kip'] =='duzenleyici')
{
	...
	if ( (!$_POST['duzenleyici']) OR (!$_POST['yduzenleyici']) OR (!$_POST['yukleme_dosya']) OR (!$_POST['yukleme_dizin']) OR (!$_POST['yukleme_boyut']) OR (!$_POST['yukleme_genislik']) OR (!$_POST['yukleme_yukseklik']) )
	{
		header('Location: ../hata.php?hata=98');
		exit();
	}
...
if ( (!$_POST['title']) OR (!$_POST['anasyfbaslik']) OR (!$_POST['syfbaslik']) OR (!$_POST['alanadi']) OR (!$_POST['f_dizin']) OR (!$_POST['fsyfkota']) OR (!$_POST['ksyfkota']) OR (!$_POST['tarih_bicimi']) OR (!$_POST['saat_dilimi']) OR (!$_POST['k_cerez_zaman']) )
{
	header('Location: ../hata.php?hata=98');
	exit();
}
