...
if (!preg_match('/'.$ayarlar['surum'].'/', $edbilgi['uyumlu_surum']))
{
...
else
{
	...
	if ($ev_hata == true)
	{
		...
		if (!isset($_GET['guncel']))
		{
			...
			VALUES ('$_GET[kur]', '1', '$eetkin', '$vt_islem', '$dosya_islem', '$dizin_islem', '$edbilgi[sistem]', '$ayarlar[surum]', '$edbilgi[eklenti_surumu]')";
		...
		else
		{
			...
			$vtsorgu = "UPDATE $tablo_eklentiler SET etkin='$eetkin', vt='$vt_islem', dosya='$dosya_islem', dizin='$dizin_islem', sistem='$edbilgi[sistem]', usurum='$ayarlar[surum]', esurum='$edbilgi[eklenti_surumu]' WHERE ad='$_GET[kur]'";
			$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());
...
$eyhakki = '&nbsp; Eklentiler Dizinine Yazma Hakkı:&nbsp;';
...
<b>Dosya Seç: &nbsp;</b><input class="formlar" name="eklenti_yukle" type="file" size="100">
<br><br><br>
...
while ( @gettype($bilgi = @readdir($yedizin)) != 'boolean' )
{
	if ( (@is_dir($yedizin_adi.$bilgi)) AND ($bilgi != '.') AND ($bilgi != '..') )
	{
		...
		else
		{
			...
			$ebilgiler = '<b>Yapımcısı:</b> <a href="'.@zkTemizle($edbilgi['eklenti_adres']).'" target="_blank">'.@zkTemizle($edbilgi['eklenti_yapimcisi']).'</a>';
			$ebilgiler .= '<br><b>Eklenti Sürümü:</b> '.$edbilgi['eklenti_surumu'];
			$ebilgiler .= '<br><b>Eklenti Tarihi:</b> '.$edbilgi['eklenti_tarihi'];


			$ebilgiler .= '<br><b>Uyumluluk:</b> '.str_replace(';',', ', $edbilgi['uyumlu_surum']);
			if (!preg_match('/'.$ayarlar['surum'].'/', $edbilgi['uyumlu_surum'])) $ebilgiler .= ' <font color="#ff0000" size="1">(Uyumsuz)</font>';
			...
			else $ebilgiler .= '<br><b>Sistem:</b> Hatalı veri';


			if ($edbilgi['tip'] == '1') $ebilgiler .= '<br><b>Eklenti Tipi:</b> Değişiklik (Mod)';
			elseif ($edbilgi['tip'] == '2') $ebilgiler .= '<br><b>Eklenti Tipi:</b> Gelişkin Eklenti';
			else $ebilgiler .= '<br><b>Eklenti Tipi:</b> Hatalı veri<br>';

			if ((isset($edbilgi['eklenti_etkin'])) AND ($edbilgi['eklenti_etkin'] == '1'))
				$ebilgiler .= '<br><b>Etkisizleştirme:</b> Destekliyor';


			if ( (isset($edbilgi['kur_veritabani'])) AND (is_array($edbilgi['kur_veritabani'])) ){
				$ebilgiler .= '<br><b>Veritabanı İşlemi:</b> Var';
				$guncelek .= '&amp;vt=1';}


			if (isset($edbilgi['tema_adi'])) $ebilgiler .= '<br><b>Tema Değişikliği:</b> '.@zkTemizle($edbilgi['tema_adi']);
			else $ebilgiler .= '<br><b>Tema Değişikliği:</b> Yok';
			if (!isset($edbilgi['tema_dizini'])) $edbilgi['tema_dizini'] = 'varsayilan';
			...
			if ( (isset($edbilgi['kur_veritabani'])) AND (is_array($edbilgi['kur_veritabani'])) AND (!isset($_GET['guncel'])) )
			{
				$ebilgiler2 .= '<b>Veritabanı Soruguları:</b> ';
				$dongu = 0;
				foreach($edbilgi['kur_veritabani'] as $a)
				{
					if ($a != '')
			...
			if ($ekl_satir['kur'] == 1)
			{
				...
				elseif ( ($ekl_satir['usurum'] == $ayarlar['surum']) AND (preg_match('/'.$ekl_satir['usurum'].'/', $edbilgi['uyumlu_surum'])) AND ($ekl_satir['esurum'] == $edbilgi['eklenti_surumu']) )
