...
$vtsorgu = "SELECT id,kullanici_adi,kullanici_kimlik,sifre,gercek_ad,posta,resim FROM $tablo_kullanicilar WHERE kullanici_kimlik='$_COOKIE[kullanici_kimlik]' LIMIT 1";
...
if ( (isset($_POST['islem_turu'])) AND ($_POST['islem_turu'] == 'normal') ):
	...
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=73');
		exit();
	}
	...
	if ((!preg_match('/^[A-Za-zğĞüÜŞşİıÖöÇç]+$/', $_POST['sehir'])) OR (strlen($_POST['sehir']) > 50))
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=36');
		exit();
	}

	if (!preg_match('/^[0-9]+$/', $_POST['dogum_gun']))
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=74');
		exit();
	}
	...
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=79');
		exit();
	}

	if ( strlen($_POST['web']) > 200)
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=75');
		exit();
	}
	...
	if ( strlen($_POST['imza']) > $ayarlar['uye_imza_uzunluk'] )
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=78');
		exit();
	}

	if ( strlen($_POST['hakkinda']) > 1000 )
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=224');
		exit();
	}
	...
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=80');
		exit();
	}
	...
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=81');
		exit();
	}

	if ( strlen($_POST['yahoo']) > 99)
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=82');
		exit();
	}

	if ( strlen($_POST['skype']) > 99)
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=83');
		exit();
	}
	...
	// Forum tema seçimi
	if ( (isset($_POST['tema_secim'])) AND (!preg_match('/^[A-Za-z0-9-_]+$/', $_POST['tema_secim'])) )
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=76');
		exit();
	}

	if ( (isset($_POST['tema_secim'])) AND (strlen($_POST['tema_secim']) > 20) )
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=77');
		exit();
	}
	...
	if ($ysk_adsoyadd[0] != '')
	{
		...
		for ($i=0; $i<$dongu_sayi; $i++)
		{
			...
			{
				header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=186');
				exit();
			}
		}
	}
	...
	elseif ( ($ayarlar['uye_resim_yukle'] == 1) AND (isset($_FILES['resim_yukle']['tmp_name']))
		AND ($_FILES['resim_yukle']['tmp_name'] != '') )
	{
		...
		if ( (isset($tip)) AND ($tip == 2) )
		{
			...
			{
				header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=84');
				exit();
			}
		}
		...
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=85');
			exit();
		}



		if ($_FILES['resim_yukle']['size'] > $ayarlar['uye_resim_boyut'])
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=86');
			exit();
		}

		if ( ($genislik > $ayarlar['uye_resim_genislik']) OR
			($yukseklik > $ayarlar['uye_resim_yukseklik']) )
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=87');
			exit();
		}

		$dosya_yolu = '../phpkf-dosyalar/resimler/yuklenen/'.rand(1111111111, 9999999999).$uzanti;

		if ( !move_uploaded_file($_FILES["resim_yukle"]["tmp_name"],$dosya_yolu) )
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=88');
			exit();
		}
...
//	UZAK RESİM YÜKLEME KISMI - BAŞI	//
	...
	elseif ( ($ayarlar['uye_resim_galerisi'] == 1) AND (isset($_POST['uzak_resim2'])) AND ($_POST['uzak_resim2'] != '') )
	{
		...
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=89');
			exit();
		}
		...
		if ($resim_boyut > $ayarlar['uye_resim_boyut'])
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=90');
			exit();
		}

		if (($genislik > $ayarlar['uye_resim_genislik']) OR ($yukseklik > $ayarlar['uye_resim_yukseklik']))
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=91');
			exit();
		}
...
else
{
	...
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=92');
		exit();
	}
	...
	//	GERÇEK AD DEĞİŞİMİ	//
	if ($_POST['gercek_ad'] != $kullanici_kim['gercek_ad'])
	{
		$vtsorgu = "UPDATE $tablo_yazilar SET yazan='$kullanici_kim[kullanici_adi];$_POST[gercek_ad]' WHERE yazan_id='$kullanici_kim[id]'";
		$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());

		$vtsorgu = "UPDATE $tablo_yazilar SET degistiren='$kullanici_kim[kullanici_adi];$_POST[gercek_ad]' WHERE degistiren_id='$kullanici_kim[id]'";
		$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());
	}
	...
	if ( ($_COOKIE['kullanici_kimlik'] == $kullanici_kim['kullanici_kimlik']) )
	{
		...
		// forum - portal tema dizini
		$temadizini_sorgu = '';
		$temadizinip_sorgu = '';
		if (isset($_POST['tema_secim'])) $temadizini_sorgu = ",temadizini='$_POST[tema_secim]'";
		if (isset($_POST['tema_secimp'])) $temadizinip_sorgu = ",temadizinip='$_POST[tema_secimp]'";


		$vtsorgu = "UPDATE $tablo_kullanicilar SET web='$_POST[web]',dogum_tarihi='$dogum_tarihi',sehir='$_POST[sehir]',gercek_ad='$_POST[gercek_ad]',resim='$kul_resim',imza='$_POST[imza]',posta_goster='$_POST[posta_goster]',dogum_tarihi_goster='$_POST[dogum_tarihi_goster]',sehir_goster='$_POST[sehir_goster]',gizli='$_POST[gizli]',icq='$_POST[icq]',aim='$_POST[aim]',msn='$_POST[msn]',yahoo='$_POST[yahoo]',skype='$_POST[skype]',cinsiyet='$_POST[cinsiyet]',hakkinda='$_POST[hakkinda]' $temadizini_sorgu $temadizinip_sorgu WHERE id='$kullanici_kim[id]' LIMIT 1";

		$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());

		header('Location: ../'.$phpkf_dosyalar['hata'].'?bilgi=10');
		exit();
	}

	else
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=94');
		exit();
	}
...
elseif ( (isset($_POST['islem_turu'])) AND ($_POST['islem_turu'] == 'sifre') ):
	...
	{
		header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=188');
		exit();
	}
	...
	if ($kullanici_kim['posta'] != $_POST['posta'])
	{
		...
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=93');
			exit();
		}
		...
		$posta_baslik = $ayarlar['site_adi'].' - E-Posta Değişikliği Onayı';

		$tiklanacak_baglanti = $protocol.'://'.$ayarlar['alanadi'].$ayarlar['dizin'].
		'/phpkf-bilesenler/kul_etkin.php?onay=eposta&kulid='.$kullanici_kim['id'].'&kulkod='.$kul_etkin_kod;

		if (!($dosya_ac = fopen($dosya,'r'))) die ('Dosya Açılamıyor');
		$posta_metni = fread($dosya_ac,3072);
		fclose($dosya_ac);

		$bul = array('{siteadi}','{kullanici_adi}','{tiklanacak_baglanti}');
		$cevir = array($ayarlar['site_adi'],$kullanici_kim['kullanici_adi'],$tiklanacak_baglanti);
		...
		$mail->YanitlamaAdres($ayarlar['site_posta'],$ayarlar['site_adi']);
	...
	if ( ($_COOKIE['kullanici_kimlik'] == $kullanici_kim['kullanici_kimlik']) )
	{
		...
		if ($es_degisti == 1)
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?bilgi=42');
			exit();
		}
		...
		elseif ($es_degisti == 2)
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?bilgi=43');
			exit();
		}
		...
		elseif ($es_degisti == 3)
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?bilgi=44');
			exit();
		}
		...
		elseif ($es_degisti == 4)
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?hata=175');
			exit();
		}
		...
		{
			header('Location: ../'.$phpkf_dosyalar['hata'].'?uyari=8');
			exit();
		}
