...
elseif ($_GET['kosul'] == 'yorum')
{
	...
	$yrmsayi = $vt->query("SELECT id FROM $tablo_pyorumlar WHERE silinmis=0 AND uye_id='$satir[id]' $onay_eksorgu") or die ($vt->hata_ver());
	...
	// yorumlar çekiliyor
	$vtsorgu = "SELECT * FROM $tablo_pyorumlar WHERE silinmis=0 AND uye_id='$satir[id]' $onay_eksorgu ORDER BY id DESC LIMIT $yrmsayfa,$yrmadim";
	$vtsonuc6 = $vt->query($vtsorgu) or die ($vt->hata_ver());
	...
	if ( (isset($kullanici_kim['id'])) AND ($yrmsayfa == 0) )
	{
		...
		elseif ($ayarlar['v-uye_resmi'] != '') $uye_yorum .= '<img src="'.$ayarlar['v-uye_resmi'].'" width="60" alt="Varsayılan Kullanıcı Resmi" title="Varsayılan Kullanıcı Resmi">';
		else $uye_yorum .= '';

		$uye_yorum .= '</td><td class="yorum_yaz" align="left" valign="top">
		<form action="javascript:Yolla(\'phpkf-bilesenler/profil2.php?kosul=yorumg&u='.$satir['id'].'\',3,\'uye_yorum\');" method="post" onsubmit="return denetle()" name="formyorum">
		...
		<div id="uye_yorum_bekle" style="position:relative; float:right; width:105px"><input class="dugme" name="yorum_gonder" type="submit" value="Gönder"></div>
	...
	while ($yorumlar = $vt->fetch_array($vtsonuc6))
	{
		...
		onclick="YorumIslem(\'phpkf-bilesenler/profil2.php?kosul=sil&amp;y='.$yorumlar['id'].'\',\'uye_yorum\',\'yorumsil'.$yorumlar['id'].'\')">Sil</a></div>';
		...
		elseif ($kullanici_kim['yetki'] == '0')
		{
			...
			elseif ($resim['yetki'] == '0')
			{
				...
				href="javascript:void(0)" onclick="YorumIslem(\'phpkf-bilesenler/profil2.php?kosul=sikayet&amp;y='.$yorumlar['id'].'\',\'yorumskyt'.$yorumlar['id'].'\')">';
		...
		else
		{
			$uye_yorum .= '<div id="yorumony'.$yorumlar['id'].'" style="position:relative;float:left;width:70px;height:18px"><a href="javascript:void(0)"
			 onclick="YorumIslem(\'phpkf-bilesenler/profil2.php?kosul=onay&amp;y='.$yorumlar['id'].'\',\'yorumony'.$yorumlar['id'].'\')">';
	...
	onMouseOut="yorum_daha_uzerine(\'yorum_daha2\',2)" onclick="Yolla(\'phpkf-bilesenler/profil2.php?kosul=yorum&u='.$satir['id'].'&s='.$yrmsayfa.'\',2,\'yorum_daha'.$yrmsayfa.'\')">
...
elseif ($_GET['kosul'] == 'yorumg')
{
	...
	{
		$cikis = '<font color="#ff0000"><b>Yolladığınız son iletinin üzerinden '.$ayarlar['yorum_sure'].' saniye geçmeden başka bir ileti gönderemezsiniz !</b></font>';
		echo $cikis;
		exit();
	}
	...
	$vtsorgu = "INSERT INTO $tablo_pyorumlar (tarih,uye_adi,uye_id,yazan,yazan_id,yazan_ip,silinmis,onay,yorum_icerik)";
...
elseif ($_GET['kosul'] == 'onay')
{
	...
	$vtsorgu = "SELECT id,onay FROM $tablo_pyorumlar WHERE id='$_GET[y]' AND silinmis=0 LIMIT 1";
	...
	$vtsorgu = "UPDATE $tablo_pyorumlar SET onay='$onay' where id='$_GET[y]'"; 
...
elseif ($_GET['kosul'] == 'sil')
{
	...
	// yorum veritabanından çekiliyor
	$vtsorgu = "SELECT uye_id,yazan_id,silinmis FROM $tablo_pyorumlar WHERE id='$_GET[y]' LIMIT 1";
	$vtsonuc1 = $vt->query($vtsorgu) or die ($vt->hata_ver());
	$yorum = $vt->fetch_array($vtsonuc1);
	...
	$vtsorgu = "UPDATE $tablo_pyorumlar SET silinmis='1' where id='$_GET[y]' LIMIT 1"; 
	$vtsonuc2 = $vt->query($vtsorgu) or die ($vt->hata_ver());
...
elseif ($_GET['kosul'] == 'sikayet')
{
	...
	// yorum veritabanından çekiliyor
	$vtsorgu = "SELECT id,yazan_id,sikayet FROM $tablo_pyorumlar WHERE id='$_GET[y]' LIMIT 1";
	$vtsonuc1 = $vt->query($vtsorgu) or die ($vt->hata_ver());
	$yorum = $vt->fetch_array($vtsonuc1);
	...
	$vtsorgu = "UPDATE $tablo_pyorumlar SET sikayet='$eksorgu' where id='$_GET[y]'"; 
	$vtsonuc2 = $vt->query($vtsorgu) or die ($vt->hata_ver());
