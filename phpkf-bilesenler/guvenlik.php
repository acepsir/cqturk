...
if ((empty($_COOKIE['kullanici_kimlik'])) OR ($_COOKIE['kullanici_kimlik'] == ''))
{
	...
	else
	{
		...
		header('Location: '.$phpkf_dosyalar['hata'].'?uyari=6&git='.$git);
		exit();
	}
}
...
elseif (isset($_COOKIE['kullanici_kimlik']))
{
	...
	$vtsorgu = "SELECT kullanici_kimlik,son_hareket,kul_ip FROM $tablo_kullanicilar WHERE kullanici_kimlik='$_COOKIE[kullanici_kimlik]' LIMIT 1";
	...
	elseif ( ($satir['kullanici_kimlik'] != $_COOKIE['kullanici_kimlik']) OR (($satir['son_hareket'] + $ayarlar['k_cerez_zaman']) < time()) )
	{
		setcookie('kullanici_kimlik', '', 0, $cerez_dizin, $cerez_alanadi);
		setcookie('yonetim_kimlik', '', 0, $cerez_dizin, $cerez_alanadi);
		header('Location: '.$phpkf_dosyalar['giris']);
...
{
	header('Location: '.$phpkf_dosyalar['hata'].'?uyari=6');
	exit();
}
