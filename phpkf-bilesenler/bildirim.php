...
if (!defined('DOSYA_AYAR')) include_once($bldrm_dizin.'../phpkf-ayar.php');
if (!defined('DOSYA_KULLANICI_KIMLIK')) include_once($bldrm_dizin.'phpkf-bilesenler/kullanici_kimlik.php');
...
if (!defined('DOSYA_GERECLER')) include $bldrm_dizin.'phpkf-bilesenler/gerecler.php';
...
if ($vt->num_rows($vtsonuc))
{
	...
	if ($bozel > 0)
	{
		...
		else
		{
			...
			$adres = $bldrm_dizin.$phpkf_dosyalar['ozel_ileti'];
	...
	if ($byorum > 0)
	{
		...
		else
		{
			...
			$adres = $bldrm_dizin.$phpkf_dosyalar['profil'].'#yorum';
	...
	if ($btsk > 0)
	{
		...
		else
		{
			...
			$adres = $bldrm_dizin.$phpkf_dosyalar['profil_degistir'].'?kosul=bildirim';
...
<script type="text/javascript" src="'.$bldrm_dizin.'phpkf-bilesenler/js/betik_bildirim.js"></script>';
}
